...
# !pip install geopandas pandas matplotlib contextily pysal requests  # Jupyter-only
...
def main():
    ...
    acs_df[["population", "median_income"]] = acs_df[
    ...
    census_tracts["area_sq_miles"] = (
        census_tracts.geometry.to_crs({"proj": "aea"}).area / 2.59e6
    )
    census_tracts["pop_density"] = (
        census_tracts["population"] / census_tracts["area_sq_miles"]
    )
    ...
    pollution = gpd.read_file(
        "houston_aqi/Current_Air_Quality_Monitoring_Stations.shp"
    ...
    census_tracts["asthma_rate"] = pd.Series(
        pd.np.random.uniform(5, 15, size=len(census_tracts))
    )
    ...
    ctx.add_basemap(
        ax, source=ctx.providers.Stamen.TonerLite, crs=census_tracts.crs.to_string()
    )
    ...
    ctx.add_basemap(
        ax, source=ctx.providers.Stamen.TonerLite, crs=pollution.crs.to_string()
    )
    ...
    # !pip uninstall -y pysal esda spaghetti libpysal geopandas  # Jupyter-only


    # --- code cell ---

    # !pip install --upgrade geopandas  # Jupyter-only
    # !pip install --upgrade libpysal  # Jupyter-only
    # !pip install --upgrade esda  # Jupyter-only
    # !pip install --upgrade pysal  # Jupyter-only
    ...
    tracts["pop_density"] = tracts["population"] / (
        tracts.geometry.area / 1e6
    )  # pop per km²
    ...
    # !pip install censusdata  # Jupyter-only
